            ...
            get
            {
                ...
                foreach (var map in maps)
                {
                    ...
                    for (int i = 0; i < desList.Count; i++)
                    {
                        ...
                        {
                            if (LockUtility.GetData(desList[i].target.Thing as Building_Door).WantedState.Private && !LockUtility.GetData(desList[i].target.Thing as Building_Door).WantedState.owners.Any(p => p.workSettings.WorkIsActive(DefDatabase<WorkTypeDef>.GetNamed("Flicker"))))
